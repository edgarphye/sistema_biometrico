...
ini_set('display_errors', 0); // Evita que salgan <br><b>… en JSON

//function rfcset($a, $b) { return $a ?: $b; }
...
class RFC
{
      ...
      public function CalcularRFC($nombre, $apellidoPaterno, $apellidoMaterno, $fechax)
      {
            $dia = substr($fechax, 8, 2);
            $mes = substr($fechax, 5, 2);
            $ano1 = substr($fechax, 0, 4);
            $ano = substr($ano1, 2, 2);
            $fecha = $dia . "/" . $mes . "/" . $ano;
            //unificamos el nombre completo de origen para generar correctamente la homoclave
            $nombrehomo = strtoupper($apellidoPaterno) . " " . strtoupper($apellidoMaterno) . " " . strtoupper($nombre);
            ...
            $this->CalcularHomoclave($nombrehomo, $fechax, $rfc);
            return $rfc;
      }
      ...
      public static function RemplazoPalabras($palabra)
      {
      ...
      public static function QuitarArticulos($palabra)
      {
      ...
      public static function QuitarArticulosNombre($palabra)
      {
            $palabra = str_replace("DEL ", "", $palabra);
            $palabra = str_replace("LAS ", "", $palabra);
            $palabra = str_replace("DE ", "", $palabra);
      ...
      public static function EsVocal($letra)
      {
            if (
...
/*
// -------------------------
// EJEMPLO DE EJECUCIÓN API
// -------------------------
try {

    $rfcObj = new RFC($_POST["nombres"] ?? "", $_POST["apellidoPaterno"] ?? "", $_POST["apellidoMaterno"] ?? "", $_POST["fecha"] ?? "");
    echo json_encode(["ok" => true, "rfc" => $rfcObj->rfc]);
} catch (Exception $e) {
