...
El sistema SHALL actualizar el nombre completo del empleado en el dispositivo biométrico cuando se modifique en la base de datos, usando `$zk->setUser()` con el mismo ID pero sin re-enrolar la huella.
...
- **WHEN** se intenta actualizar un empleado en un dispositivo que no responde
- **THEN** el sistema retorna error indicando que el dispositivo no está disponible
