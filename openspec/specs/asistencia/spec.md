...
## Purpose
Gestionar el registro diario de asistencia con captura biométrica y manual, clasificación automática del tipo de asistencia, cálculo de horas laborables, validación jerárquica y exportación a Excel y PDF.
...
### Requirement: Campo de validación por RH
El sistema SHALL usar campo validado_por_rh para gestionar el estado de validación de registros de asistencia.

#### Scenario: Estados de validación
- **WHEN** se procesa un registro de asistencia
- **THEN** el sistema asigna uno de: MIGRATION_AUTO (auto-validado), REVISION_MANUAL (requiere revisión), RH (validado por RH)

#### Scenario: Estado final post-migración
- **WHEN** se completa la migración y limpieza
- **THEN** el sistema tiene 10,187 MIGRATION_AUTO, 270 REVISION_MANUAL (solo salida sin entrada), 1,782 RH
