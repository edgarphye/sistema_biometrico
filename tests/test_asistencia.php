...
if ($empleado) {
    // Registrar entrada
    $result = $asistencia->registrarEntrada($empleado['id'], 1, $datos['type'], $datos['data'], $datos['quality_score'], $datos['metadata'], 0.5);
    echo 'Entrada registrada: ' . ($result ? 'SI' : 'NO') . PHP_EOL;

    // Calcular retardo
    $retardo = $asistencia->calcularRetardo('09:15:00', $empleado['id'], date('Y-m-d')); // Simular llegada a las 9:15
